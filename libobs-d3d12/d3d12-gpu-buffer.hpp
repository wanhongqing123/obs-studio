...
class gs_gpu_buffer : public gs_gpu_resource
{
public:
    virtual ~gs_gpu_buffer() { Destroy(); }
...
protected:
	...
	gs_gpu_buffer(void) : m_BufferSize(0), m_ElementCount(0), m_ElementSize(0)
    {
...
inline D3D12_VERTEX_BUFFER_VIEW gs_gpu_buffer::VertexBufferView(size_t Offset, uint32_t Size, uint32_t Stride) const
{
...
inline D3D12_INDEX_BUFFER_VIEW gs_gpu_buffer::IndexBufferView(size_t Offset, uint32_t Size, bool b32Bit) const
{
...
class gs_byte_address_buffer : public gs_gpu_buffer
{
public:
    virtual void CreateDerivedViews(void) override;
};

class gs_indirect_args_buffer : public gs_byte_address_buffer
{
public:
	gs_indirect_args_buffer(void)
    {
    }
};

class gs_structured_buffer : public gs_gpu_buffer
{
public:
    virtual void Destroy(void) override
    {
        m_CounterBuffer.Destroy();
	gs_gpu_buffer::Destroy();
    }

    virtual void CreateDerivedViews(void) override;

    gs_byte_address_buffer& GetCounterBuffer(void) { return m_CounterBuffer; }
...
private:
	gs_byte_address_buffer m_CounterBuffer;
};

class gs_typed_buffer : public gs_gpu_buffer
{
public:
	gs_typed_buffer( DXGI_FORMAT Format ) : m_DataFormat(Format) {}
    virtual void CreateDerivedViews(void) override;
